...
function processCurrentInstruction(draw) {
    ...
    if (status.execution_mode === ExecutionMode.User && instruction.properties?.includes('privileged')) {
        throw new Error("💀 Can't execute privileged instruction '" + instruction.name + "' in User mode.");
    }
